...
function scoreText(piece) {
  const s = piece.scores;
  return `selection z ${s.selection_score >= 0 ? "+" : "−"}`
    + `${Math.abs(s.selection_score).toFixed(2)}`
    + ` · CLaMP3 text ${s.clamp3_text_cos.toFixed(3)}`
    + ` · tuple ${s.clamp3_tuple_cos.toFixed(2)}`
    + ` · CLAP ${s.clap_score.toFixed(2)}`
    + ` · key ${mark(s.key_match)} tempo ${mark(s.tempo_match)}`
    + ` · judge: `
    + s.judge.map(entry => `${entry.value} ${mark(entry.match)}`).join(" / ");
}

// one live view per roll; a card holds the chosen view and the rejected view
function buildSide(piece, side) {
  const column = document.createElement("div");
  column.className = `side ${side}`;

  const heading = document.createElement("div");
  heading.className = "side-title";
  heading.textContent = piece.label;

  const subtitle = document.createElement("div");
  subtitle.className = "subtitle";
  subtitle.textContent =
    `seed ${piece.seed} · ${formatTime(piece.duration_sec)} · `
    + `${piece.n_tokens} tokens · ${piece.notes.length} notes`
    + (piece.ended_naturally ? " · ended on <E>" : " · truncated at the token cap");
  ...
  line.className = `score-line ${side}`;
  line.textContent = scoreText(piece);

  const foot = document.createElement("div");
  foot.className = "card-foot";
  const midiLink = document.createElement("a");
  midiLink.className = "midi-link";
  midiLink.href = piece.midi;
  midiLink.textContent = piece.midi;
  foot.append(midiLink);

  column.append(heading, subtitle, audio, stack, line, foot);

  const view = {piece, canvas, cursor, audio, layout: null, frame: null};
  views.push(view);
  ...
  const start = () => {
    // only one roll plays at a time, across every card
...
function buildCard(pair, position) {
  const card = document.createElement("article");
  card.className = "card";
  card.id = `pair${position}`;
  ...
  tag.textContent = `margin ${pair.margin.toFixed(2)} z · K = ${pair.n_candidates}`;
  const pos = document.createElement("span");
  pos.className = "position";
  pos.textContent = `${position} / ${pairs.length}`;
  headRow.append(tag, pos);

  const title = document.createElement("h1");
  title.textContent = `prompt ${pair.name} — ${pair.emotion} · ${pair.genre}`;

  const panel = document.createElement("div");
  panel.className = "header-panel";
  panel.append(
    field("emotion", pair.emotion),
    field("genre", pair.genre),
    field("accompaniment", pair.accompaniment),
    field("performance", pair.performance),
    field("key", pair.key),
    field("tempo class", pair.tempo),
  );
  ...
  sentimentName.textContent = "sentiment (FLAN-T5 prefix)";
  const sentimentText = document.createElement("p");
  sentimentText.textContent = pair.sentiment;
  sentiment.append(sentimentName, sentimentText);

  header.append(headRow, title, panel, sentiment);

  const body = document.createElement("div");
  body.className = "pair-body";
  const chosen = buildSide(pair.chosen, "chosen");
  const rejected = buildSide(pair.rejected, "rejected");
  body.append(chosen.column, rejected.column);

  card.append(header, body);
  cards.appendChild(card);

  [chosen.view, rejected.view].forEach(view => {
    draw(view);
    drawCursor(view, 0);
  });
}
...
pairs.forEach((pair, index) => {
  ...
  name.textContent = `prompt ${pair.name} · margin ${pair.margin.toFixed(2)}`;
  const info = document.createElement("small");
  info.textContent = `${pair.emotion} · ${pair.genre} · ${pair.key}`;
  link.append(name, info);
  link.addEventListener("click", () => {
    document.getElementById(`pair${index + 1}`)
      .scrollIntoView({behavior: "smooth", block: "start"});
  });
  contents.appendChild(link);
  buildCard(pair, index + 1);
});
