...
function renderScores(view) {
  view.scores.replaceChildren(...view.prompt.candidates.map((piece, index) => {
    const row = scoreLine(piece);
    if (index === view.index) row.classList.add("active");
    return row;
  }));
}
