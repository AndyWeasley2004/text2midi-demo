...
function judgeCell(entry) {
  const cell = document.createElement("td");
  cell.className = entry.match ? "judge-cell hit" : "judge-cell miss";
  const value = document.createElement("span");
  value.className = "judge-value";
  value.textContent = entry.value;
  const prob = document.createElement("span");
  prob.className = "judge-prob";
  prob.textContent = ` (${entry.prob.toFixed(2)}) ${mark(entry.match)}`;
  cell.append(value, prob);
  return cell;
}

function promptCell(text) {
  const cell = document.createElement("td");
  cell.className = "judge-cell prompt";
  cell.textContent = text;
  return cell;
}

function judgeRow(label, cells, className) {
...
function renderJudges(view) {
  const table = document.createElement("table");
  table.className = "judge-table";
  const head = document.createElement("thead");
  const headRow = document.createElement("tr");
  ["judge", "accompaniment", "performance"].forEach(text => {
  ...
  body.appendChild(judgeRow(
    "prompt",
    [promptCell(view.prompt.accompaniment), promptCell(view.prompt.performance)],
    "prompt-row"));
  for (const entry of view.piece.judges) {
    body.appendChild(judgeRow(
      entry.label, [judgeCell(entry.accompaniment), judgeCell(entry.performance)]));
  }
  table.append(head, body);
  view.scores.replaceChildren(table);
}

function renderTabs(view) {
  view.tabs.replaceChildren(...view.prompt.candidates.map((entry, index) => {
...
function selectCandidate(view) {
  view.piece = view.prompt.candidates[view.index];
  renderTabs(view);
  renderJudges(view);
  view.subtitle.textContent =
    `seed ${view.piece.seed} · ${formatTime(view.piece.duration_sec)} · `
    + `${view.piece.n_tokens} tokens · `
    + `${view.piece.notes.length} notes`
    + (view.piece.ended_naturally ? " · ended on <E>" : " · truncated at the token cap");
...
function buildCard(prompt, position) {
  ...
  tag.textContent = `midi_gen_lyria_sft_v2 / final · best of ${prompt.n_candidates}`;
  ...
  title.textContent = `prompt ${prompt.name} — ${prompt.emotion} · ${prompt.genre}`;
  ...
  tabs.setAttribute("aria-label", "Rank");
  ...
  header.append(headRow, title, subtitle, tabs, panel, sentiment);
