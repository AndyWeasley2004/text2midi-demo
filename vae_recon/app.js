...
function metricText(piece) {
  const m = piece.metrics;
  return `note F1 ${m.note_f1.toFixed(3)}`
    + ` (P ${m.note_p.toFixed(3)} / R ${m.note_r.toFixed(3)})`
    + ` · velocity ${m.vel_exact.toFixed(3)}`
    + ` · duration ${m.dur_within50.toFixed(3)}`
    + ` · pedal F1 ${m.pedal_f1.toFixed(3)}`
    + ` · notes ${m.n_notes_orig} → ${m.n_notes_recon}`;
}

// one live view per roll; a card holds the original view and the reconstruction view
function buildSide(piece, side) {
  ...
  subtitle.textContent =
    `${formatTime(piece.duration_sec)} · ${piece.notes.length} notes`;
  ...
  column.append(heading, subtitle, audio, stack, foot);
...
function buildCard(piece, position) {
  ...
  tag.textContent = piece.stage;
  const pos = document.createElement("span");
  pos.className = "position";
  pos.textContent = `${position} / ${pieces.length}`;
  headRow.append(tag, pos);

  const title = document.createElement("h1");
  title.textContent = `${piece.stage} · ${piece.piece_id} · ${piece.seconds} s · `
    + `${piece.density.toFixed(1)} note tokens per 5 s`;

  const line = document.createElement("div");
  line.className = "score-line recon";
  line.textContent = metricText(piece);

  header.append(headRow, title, line);

  const body = document.createElement("div");
  body.className = "pair-body";
  const orig = buildSide(piece.orig, "orig");
  const recon = buildSide(piece.recon, "recon");
  body.append(orig.column, recon.column);

  card.append(header, body);
  cards.appendChild(card);

  [orig.view, recon.view].forEach(view => {
    draw(view);
    drawCursor(view, 0);
  });
}
...
pieces.forEach((piece, index) => {
  ...
  name.textContent = `${piece.stage} · ${piece.piece_id}`;
  const info = document.createElement("small");
  info.textContent = `${piece.density.toFixed(1)} tok/5 s · note F1 `
    + `${piece.metrics.note_f1.toFixed(3)}`;
  link.append(name, info);
  link.addEventListener("click", () => {
    document.getElementById(`piece${index + 1}`)
      .scrollIntoView({behavior: "smooth", block: "start"});
  });
  contents.appendChild(link);
  buildCard(piece, index + 1);
});
