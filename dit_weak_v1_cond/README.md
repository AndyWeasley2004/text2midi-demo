# Latent DiT — weak stage, conditional samples
...
with classifier-free guidance 3.0 and 50 Euler steps, seeds 101/102/103, and
are decoded back to MIDI through the grammar-constrained decoder
`inference/latent_decode.py`. Rendered with the canvas pianoroll (synced
playhead, click-to-seek, sustain pedal both drawn and audible).
...
python -m inference.sample_dit \
  ...
  --prompts-json demos/dit_weak_v1_cond/prompts.json \
  --n-seeds 3 \
  --output-dir demos/dit_weak_v1_cond
```
...
LD_PRELOAD=/lib/x86_64-linux-gnu/libffi.so.7 OMP_NUM_THREADS=8 \
  python demos/dit_weak_v1_cond/build.py
```
