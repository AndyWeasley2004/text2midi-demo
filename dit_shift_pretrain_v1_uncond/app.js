...
function buildCard(piece, position) {
  ...
  tag.textContent = `dit_shift_pretrain_v1 / final (EMA) · guidance ${piece.cfg} · ${piece.steps} steps`;
  ...
  title.textContent = `sample ${piece.index} — source piece ${piece.source}`;
  const plan = document.createElement("div");
  plan.className = "plan-note";
  plan.textContent = "key, tempo class, section plan and length borrowed from that "
    + "pretrain-validation piece; attributes null, sentiment context empty";
  const subtitle = document.createElement("div");
  subtitle.className = "subtitle";
  subtitle.textContent =
    `${formatTime(piece.duration_sec)} · ${piece.notes.length} notes · `
    + `${piece.n_tokens} decoded tokens · plan ${piece.plan_windows}`
    + ` windows (${formatTime(piece.plan_sec)}) · seed ${piece.seed}`;

  const panel = document.createElement("div");
  panel.className = "header-panel";
  panel.append(
    field("source piece", piece.source),
    field("key", piece.key),
    field("tempo class", piece.tempo),
    field("length", `${piece.n_seconds} s`),
  );

  header.append(headRow, title, plan, subtitle, panel);
...
pieces.forEach((piece, index) => {
  ...
  name.textContent = `sample ${piece.index} · ${piece.source}`;
  const info = document.createElement("small");
  info.textContent = `${piece.key} · ${piece.tempo} · ${piece.n_seconds} s`;
  link.append(name, info);
  link.addEventListener("click", () => {
    document.getElementById(`piece${piece.index}`)
